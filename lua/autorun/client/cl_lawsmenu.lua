...
height = ScrH() -- useless (It's there just in case)
width = ScrW() -- useless (It's there just in case)
...
local function law_menu()
	...
	lclose.Paint = function( pnl, w, h ) 
		draw.RoundedBoxEx( 8, 0, 0, w, h, Color( 200, 40, 40 ),false,true,true,false)
	end
	lclose.DoClick = function() lmenu:Remove() end

	local llist = vgui.Create("DListView", lmenu)
	llist:SetPos(3, 24 + 3)
	llist:SetSize(lmenu:GetWide() - 6, lmenu:GetTall() - 24 - 6 - 30) -- width 524 (1600x1024)
	...
	if LocalPlayer():getJobTable().mayor then
		...
		lrem.DoClick = function() law_remove() end
		function lrem:Paint( w, h ) -- Remove Laws Button Color
